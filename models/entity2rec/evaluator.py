...
class Evaluator(object):
    def __init__(self, implicit=False, all_unrated_items=True):
    ...
    def _parse_data(self, train, validation=None):
        """
        ...
        self.all_items = list(set(all_train_items))
    ...
    def features(self, recommender, training, validation=None, n_users=False, n_jobs=4,
                 supervised=True, max_n_feedback=False):

        # reads .dat format
        self._parse_data(training, validation=None)
    ...
    def write_candidates(self, training, test, users_folder, candidates_folder, index_file, data='test',
                         validation=None):
    ...
    def compute_item_to_item_similarity(self, recommender, training, test, dataset, validation=None, n_users=False,
                                        n_jobs=4,
        ...
        if not property_specif_emb:
            ...
            for seed_item in item_index.keys():
                ...
                for candidate_item in item_index.keys():
                    ...
                    d[candidate_item] = np.mean(
                        [user_item_relatedness[u_id, candidate_item] for u_id in users_liking_item], axis=0)
